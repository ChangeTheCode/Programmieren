...
void scheduler_suspend(int threadID){
	threadList[threadID].state = BLOCKED; // set thread to blocked, there no ressources anymore
}

void scheduler_continueThread(int toStartedThreadId){
	int nextThreadID = scheduler_getNextThreadToRun();
	if(nextThreadID >= 0) {
		threadList[toStartedThreadId].state = WAITING;
	}else{
		threadList[0].state = WAITING; // 0 is the main thread, this must always running
	}
}
...
void scheduler_runNextThread() {
	...
	case WAITING:
		if(currThread == -1){
			...
			longjmp(threadList[currThread].context, 1); // fehler hier geht was bei springen schief, keine Ahnung wiso und was, müsste eigentlich die Richtigen
														//wert gespeichert haben und zum ersten Thread springen und den ausführen macht das aber nicht
		}else{
